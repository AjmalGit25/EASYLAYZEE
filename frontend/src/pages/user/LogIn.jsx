...
function Login() {
  ...
  const { loginUser, syncGuestCart } = useUserAuth();
  ...
  const handleFormData = async () => {
    try {
      ...
      setError(error.response?.data?.message || "Something went wrong in Signin!");
    }
  };

  const inputClass = "w-full border border-gray-200 rounded-lg p-2.5 pl-10 outline-none text-sm transition-all duration-200 bg-gray-50 hover:border-indigo-300 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100 focus:bg-white";
      ...
      <div className="flex items-center justify-center min-h-screen px-4">
        <div className="w-full max-w-md">
          ...
          <div className="bg-white rounded-2xl shadow-xl overflow-hidden">
            ...
            {/* Card Header */}
            <div className="bg-linear-to-r from-indigo-600 via-purple-600 to-pink-500 px-8 py-8 text-center">
              <p className="text-white/80 text-3xl font-black tracking-wide">
                ⚡ EASY<span className="text-yellow-300">LAYZEE</span>
              </p>
              <p className="text-white/70 text-sm mt-1">Sign in and grab your delicious food</p>
            </div>
            ...
            <div className="px-8 py-7">
              <form onSubmit={(e) => { e.preventDefault(); handleFormData(); }} className="space-y-5">
                ...
                <button
                  type="submit"
                  className="w-full py-2.5 rounded-lg text-white font-semibold text-sm bg-linear-to-r from-indigo-600 via-purple-600 to-pink-500 hover:opacity-90 transition-opacity duration-200 cursor-pointer shadow-md mt-2"
                >
                  Sign In
                </button>
              </form>
